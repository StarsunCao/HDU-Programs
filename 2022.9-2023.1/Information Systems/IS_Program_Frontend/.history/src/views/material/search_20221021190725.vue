    ...
    <el-table :data="data1" style="width: 100%;max-height:80% ;">
      ...
      <el-table-column prop="picture" label="picture" width="300px">
        <template v-slot="scope">
          <el-image :src="'http://192.168.31.73:8080/images/'+scope.row.picture" style="width:210px;height:150px">
          </el-image>
        </template>
      </el-table-column>
      ...
      <el-table-column label="operation" width="300px">
        <div slot-scope="scope">
          <el-button type="success" circle @click="rowClick(scope.row.apartment_ID)" icon="el-icon-tickets"></el-button>
          <el-button type="success" circle @click="edit(scope.row.apartment_ID)" icon="el-icon-edit"
            v-if="(uid == scope.row.user_ID)||(JSON.parse(admin) == true)"></el-button>
          <el-button type="info " circle @click="del(scope.row.apartment_ID)" icon="el-icon-delete"
            v-if="(uid == scope.row.user_ID)||(JSON.parse(admin) == true)"></el-button>
        </div>
      </el-table-column>
    </el-table>
