...
for i in range(0, 2):
    ...
    comment = driver.find_elements(By.XPATH, r'//div[@class="col col-4"]/a')
    sale = driver.find_elements(By.XPATH, r'//p[@class="deal-cnt"]')

    # links.extend(link)
    for j in range(0,44):
        names.extend(name[j].text)
        prices.extend(price[j].text)
        locations.extend(location[j].text)
        comments.extend(comment[j].text)
        sales.extend(sale[j].text)
