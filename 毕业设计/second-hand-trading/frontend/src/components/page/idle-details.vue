    ...
    export default {
        ...
        methods: {
            createOperation(type){
                let userId=this.getCookie('shUserId');
                if(userId != '' && this.isMaster == false){
                        this.operation.userId = userId;
                        this.operation.idleId = this.idleItemInfo.id;
                        this.operation.type = type;
                        this.$api.createOperation(this.operation).then(res=>{console.log(res);})
                    }
            },
            getAllIdleMessage(){
