...
import com.second.hand.trading.server.service.impl.IdleItemServiceImpl;
...
public class IdleItemController {
    ...
    @Autowired
    private IdleItemServiceImpl idleItemServiceImpl;
    ...
    @GetMapping("countbylabel")
    public int[] countByLabel(){
        int list[] = new int[5];
        for(int i = 0;i<5;i++){
            list[i] = idleItemServiceImpl.countIdleItemByLabel(i+1);
        }
